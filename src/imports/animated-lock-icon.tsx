...
// Framer Motion internally sub-steps its spring solver; a single
// Euler step per frame with high stiffness over large distances
// converges in <10 frames and feels like a teleport.
...
function useSpringValue(initial) {
  ...
  const animateTo = useCallback((target, { stiffness = 400, damping = 25, mass = 1, onComplete } = {}) => {
    ...
    const tick = (now) => {
      // Use real delta time, clamped to avoid spiral of death
      const prev = prevTimeRef.current ?? now;
      prevTimeRef.current = now;
      const frameDt = Math.min((now - prev) / 1000, 0.032); // cap at ~30fps
      ...
      // Settle check — tight threshold so we don't snap early
...
// FIX: viewBox is centered on lock content (0 0 56 70) with overflow:visible
// so the open shackle can extend above without clipping. transform-origin uses
// absolute SVG coordinates (not percentages which are viewport-relative).

function AnimatedLockIcon({ isLocked, opacity }) {
  ...
  // The shackle path bounding box is ~x:17.5-38.5, y:7-24.5
  // Framer's originX:"70%" originY:"100%" means:
  //   x = 17.5 + (38.5-17.5)*0.7 = 32.2
  //   y = 24.5 (bottom of path)
  // In CSS on SVG, transform-origin percentages are viewport-relative,
  // so we MUST use absolute px values in the SVG coordinate system.
...
export default function SwipeToLock() {
  ...
  const onPointerMove = useCallback((e) => {
    ...
    // Slight elastic feel at edges (matches dragElastic: 0.05)
  ...
  const onPointerUp = useCallback(() => {
    if (!isDragging) return;
    setIsDragging(false);
    const current = x.ref.current;

    if (current >= LOCK_THRESHOLD) {
      x.animateTo(MAX_DRAG, {
        stiffness: 400,
        damping: 25,
        onComplete: () => {
          setIsLocked(true);
