...
const GAP_ANGLE = 0.05; // actual angular gap — shapes won't bleed past this
          ...
          {arcs.map((arc) => {
            ...
            const sw = isSelected
              ...
              : isDimmed
                ? STROKE_SHRUNK
                : STROKE_NORMAL;
            // Normal inner/outer edges
            const normalInner = RADIUS - STROKE_NORMAL / 2;
            const normalOuter = RADIUS + STROKE_NORMAL / 2;

            // Selected: keep inner edge fixed, expand outward only
            // Dimmed: shrink symmetrically
            const innerR = isDimmed ? RADIUS - sw / 2 : normalInner;
            const outerR = isSelected ? normalOuter + (STROKE_EXPANDED - STROKE_NORMAL) : isDimmed ? RADIUS + sw / 2 : normalOuter;
